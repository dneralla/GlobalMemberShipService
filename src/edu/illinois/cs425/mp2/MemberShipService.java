...
public class MemberShipService {
	...
	// TODO
	private final MemberNode joinNode = null;
	...
	public MemberShipService(InetAddress hostAddress, int port,
			MemberNode multicastGroup) {
	...
	public void joinGroupRequest() throws IOException {
		...
		DatagramPacket packet = new DatagramPacket(b, b.length,
				joinNode.getHostAddress(), joinNode.getHostPort());
	...
	public void serviceJoinGroup(MemberNode node) {
		try {
			if (node != null) {
				boolean b = (this.memberNode).equals(this
	...
	public void leaveGroupRequest() throws IOException {
		...
		DatagramPacket packet = new DatagramPacket(b, b.length, this
				.getHeartBeatReceiver().getHostAddress(), this
				.getHeartBeatReceiver().getHostPort());
		getMemberSocket().send(packet);
	}
	...
	public void sendLeaveMulticastUpdate(byte[] b) throws IOException {
		// TODO
		DatagramPacket packet = new DatagramPacket(b, b.length,
				this.multicastGroup.getHostAddress(),
				this.multicastGroup.getHostPort());
		getMemberSocket().send(packet);
	...
	public void receiveMulticastupdate() throws IOException
{
	...
	public void sendHeartBeatInfo(MemberNode toNode, MemberNode neighborNode)
			throws IOException {
		byte b[] = nodeToBytes(neighborNode);
		DatagramPacket packet = new DatagramPacket(b, b.length,
				toNode.getHostAddress(), toNode.getHostPort());
		getMemberSocket().send(packet);
	...
	public void sendHeartBeat() throws IOException { // send node info
		byte b[] = this.nodeToBytes(this.memberNode);
		DatagramPacket packet = new DatagramPacket(b, b.length, this
				.getHeartBeatReceiver().getHostAddress(), this
				.getHeartBeatReceiver().getHostPort());
	...
	public byte[] nodeToBytes(MemberNode node) {
		// TODO make this nodetoBytes(node,Message) message indicates join or
		// etc etc
		return null;
	}
